...
public class Map implements Serializable {
    ...
    private Position cheatStore = null;
    ...
    protected void setRANDOM(Random seed) {
        RANDOM = seed;
    }
    ...
    private boolean addRoom() {
        ...
        //cheat testing
        Position currentCheat = draft.getCheat();
        cheatTest(currentCheat);
        cheatStore = currentCheat;
    ...
    protected void cheatTest(Position currentCheat) {
        if (cheatStore == null) {
            cheatStore = currentCheat;
            return;
        }
        pathFinder p = new pathFinder(currentCheat, world, RANDOM);
        p.cheatPath(cheatStore);
        cheatStore = currentCheat;
    }
    ...
    /**takes in entrance position, make a pathFinder and let
     * it run till done
     * @param entrance
     */
    private void makeHallwayhelper(Position entrance) {
        Position orig = new Position(entrance.xPos, entrance.yPos); //keeps track of original entrance
        ...
        pathFinder mazeRunner = new pathFinder(orig, d, world, RANDOM);
        while (!mazeRunner.finished()) {
            mazeRunner.adventure();
        }
    }
    ...
    private Area aroundMe(Position p) {
        Position topLeft = new Position(p.xPos - 1, p.yPos + 1);
        Area around = new Area(topLeft, 3, 3);
        around.fillEdge(Game.WIDTH, Game.HEIGHT);
        return around;
    }

    private Area oneLessAll() {
        Position topLeftMap = new Position(1, Game.HEIGHT - 2);
        Area all = new Area(topLeftMap, Game.HEIGHT - 2, Game.WIDTH  - 2);
        all.fillArea(Game.WIDTH, Game.HEIGHT);
        return all;
    }

    //resource consuming
    protected void sealer() {
        Area all = oneLessAll();
        while (!all.isEmpty()) {
            Position temp = all.first();
            if (getTile(temp) == NOTHING) {
                Area around = aroundMe(temp);
                while (!around.isEmpty()) {
                    Position checking = around.first();
                    if (getTile(checking) == FLOOR) {
                        tilePlacer(checking, WALL);
                    }
                }
            }
        }
